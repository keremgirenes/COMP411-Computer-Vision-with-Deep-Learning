...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        pool_height = ((height - 2) // 2) + 1   # 2x2 max-pool given in definiton
        pool_width = ((width - 2) // 2) + 1

        self.params['W1'] = np.random.normal(loc= 0.0, scale=weight_scale, size=(num_filters, C, filter_size, filter_size))
        self.params['b1'] = np.zeros(num_filters)

        self.params['W2'] = np.random.normal(loc= 0.0, scale=weight_scale, size=(num_filters * pool_height * pool_width, hidden_dim))
